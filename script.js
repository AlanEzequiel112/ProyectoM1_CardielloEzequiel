...
// Genera la paleta y guarda los colores
function generarPaleta() {
  const tamaño = parseInt(seleccionTamaño.value);
  colores = [];

  for (let i = 0; i < tamaño; i++) {
    colores.push(generarHex());
  }

  mostrarPaleta();
}

// Renderiza desde el array (NO genera colores nuevos)
function mostrarPaleta() {
  ...
  colores.forEach(color => {
    ...
    codigo.textContent =
      formatoActual === "HEX" ? color : hexAHSL(color);
...
botonFormato.addEventListener("click", () => {
  ...
  mostrarPaleta(); // ← ahora solo re-renderiza
});